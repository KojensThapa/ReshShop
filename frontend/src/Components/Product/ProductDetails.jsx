...
const selectProduct = {
  ...
  price: 1200,
  orginalPrice: 1400,
  description: "Stylish jacket and this discription",
  brand: "FashionHub",
  material: "Lether",
  sizes: ["S", "M", "L", "XL"],
  colors: ["Red", "Black"],
...
const ProductDetails = () => {
  ...
  const handleQuantityChange = (action) => {
    if (action === "plus" && quantity < 10) setQuantity((prev) => prev + 1);
    if(action === "minus" && quantity > 1) setQuantity((prev) => prev -1)
  }

  const handleAddToCart = () =>{
    if(!selectedColor || !selectedSize ) {
      toast.error("Please Select the color and Size of the Product.", {
        duration: 1000, 
      });
      return;
    }
        ...
        <div className="flex flex-col md:flex-row">
          ...
          <div className="md:w-1/2 md:ml-10">
            ...
            <p className="text-lg text-gray-600 mb-1 line-through">
              Nrp:{" "}
              {selectProduct.orginalPrice && `${selectProduct.orginalPrice}`}
            </p>
            <p className="text-xl text-gray-500 mb-2">
              Nrp: {selectProduct.price}
            </p>
            ...
            {/* Increment and decrement the quantity section*/}
            <div className="mb-6">
            ...
            className={`w-full bg-black text-white rounded py-2 px-6 mb-4 hover:bg-custom-red font-semibold 
              ${isButtonDisabled ? "cursor-not-allowed opacity-70" : "hover:bg-custom-red"}`}>
